	...
	for len(src) > 0 {
		...
		if err != nil {
			return hryzun.NewStatus(
				...
				"pletyvo-node/store/pebble: cannot read next DeliveryMessage field",
			)
		}
		...
		case 1:
			...
			if !ok {
				return hryzun.NewStatus(
					pletyvo.CodeInternal,
					"pletyvo-node/store/pebble: cannot read DeliveryMessage body",
				)
			}
		...
		case 2:
			...
			if !ok {
				return hryzun.NewStatus(
					pletyvo.CodeInternal,
					"pletyvo-node/store/pebble: cannot read DeliveryMessage auth",
				)
			}
